...
casted_data AS (
    SELECT
        ...
        -- payment_value is numeric/float in source; cast/round directly to desired precision
...
final AS (
    ...
    FROM casted_data c
    CROSS JOIN bounds b
    WHERE c.payment_installments > 0      -- installment cannot be 0
      AND c.payment_value > 0             -- remove zero payments (test/cancelled transactions)
      AND c.payment_value BETWEEN b.lower_bound AND b.upper_bound  -- remove outliers
)
