-- Schema DDL Statements for kpi_executive_summary
-- Generated: 20251221_140710
-- Source: s3://movielens-elt-project/gold/kpi_executive_summary/
-- ============================================================================
...
-- S3 TABLES CTAS
-- ----------------------------------------------------------------------------
-- S3 Tables (Iceberg) using CTAS for kpi_executive_summary
-- Step 1: Create external table (use regular_s3.sql)
-- Step 2: Create S3 Table from it
CREATE TABLE s3_tables_catalog.database_name.kpi_executive_summary_iceberg
WITH (
  table_type = 'ICEBERG',
  location = 's3://your-s3-tables-bucket/kpi_executive_summary/',
  is_external = false,
  format = 'PARQUET'
)
AS SELECT * FROM database_name.kpi_executive_summary;


-- S3 TABLES DIRECT
-- ----------------------------------------------------------------------------
-- Direct S3 Tables (Iceberg) creation for kpi_executive_summary
CREATE TABLE s3_tables_catalog.database_name.kpi_executive_summary_iceberg (
  total_users BIGINT,
  total_movies BIGINT,
  total_ratings BIGINT,
...
-- Then insert data
INSERT INTO s3_tables_catalog.database_name.kpi_executive_summary_iceberg
SELECT * FROM database_name.kpi_executive_summary;
